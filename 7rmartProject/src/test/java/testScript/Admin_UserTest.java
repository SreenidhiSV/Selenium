...
public class Admin_UserTest extends Base {
	...
	public void adminCanAddNewUsers() throws IOException {
		...
		/*
		 * Admin_UserPage admin = new Admin_UserPage(driver); admin.clickOnAdminUsers();
		 * admin.manageUsers(); admin.newButton(); FakerUtility faker=new
		 * FakerUtility(); String username2=faker.createRandomUsername(); String
		 * password2=faker.createRandomPassword();
		 * admin.enterUsernameOnUsernameField(username2);
		 * admin.enterPasswordOnPasswordField(password2);
		 * admin.selectUserType(usertype); admin.clickOnSaveButton();
		 */
	...
	public void adminCanSearchUsers() throws IOException {
		...
		homePage = login.clickOnSigninButton();
		adminPage = homePage.clickOnAdminUsers();
		adminPage.manageUsers().clickOnSearchButton().clickOnSearchUsernameField(username2).searchUserType(usertype)
				.clickOnSearchValue();
		/*
		 * Admin_UserPage admin = new Admin_UserPage(driver); admin.clickOnAdminUsers();
		 * admin.manageUsers(); admin.clickOnSearchButton();
		 * admin.clickOnSearchUsernameField(username2); admin.selectUserType(usertype);
